...
def ItemCF_Recommend(sim_item_corr, interacted_items, interacted_time, user_id, top_k, item_num, time_max):
    rank = {}
...
def get_itemcf_recall(data, target_df, df, time_max, topk=200, rec_num=100, use_iif = False):
    ...
    sim_item_corr, user_item_dict, user_time_dict, user_price_dict = get_sim_item(df, 'customer_id', 'article_id', use_iif=use_iif)
    ...
    for cust, hist_arts, dates in tqdm(data[['customer_id', 'article_id', 't_dat']].values):
        ...
        rec = ItemCF_Recommend(sim_item_corr, hist_arts, dates, cust, topk, rec_num, time_max)
...
def generate_recall_samples_test(data, date, cg, last_days=7, topk=1000, recall_num=100, use_iif=False):
    ...
    sim_item_corr, user_item_dict, user_time_dict, user_price_dict = get_sim_item(data[data.t_dat >= last_month_date], 'customer_id', 'article_id',
                                                                                  use_iif=use_iif)
...
recall_num, topk = 300, 2000

use_iif = True # False

print('train')
generate_recall_samples(transactions_train, '2020-09-15', last_days=7, topk=topk, recall_num=recall_num, dtype='train', use_iif=use_iif)

print('valid')
generate_recall_samples(transactions_train, '2020-09-22', last_days=7, topk=topk, recall_num=recall_num, dtype='valid', use_iif=use_iif)
...
print('test')
generate_recall_samples_test(transactions_train, '2020-09-22', cg2, last_days=7, topk=topk, recall_num=recall_num, use_iif=use_iif)
