...
def process(customers, articles):
    ...
    # tmp = transactions_train.merge(articles[['article_id', 'index_group_no']], on='article_id', how='left').groupby(
    #     ['customer_id', 'index_group_no'])['article_id'].agg('count').reset_index()
    ...
    '''
    cols = ['index_group_no', 'product_type_no', 'product_group_name', 'graphical_appearance_no',
            'colour_group_code', 'department_no', 'section_no', 'garment_group_no']
    transactions = transactions_train[transactions_train.t_dat <= '2020-09-15']
    for col in cols:
        tmp = transactions.merge(articles[['article_id', col]], on='article_id', how='left').groupby(
            ['customer_id', col])['article_id'].agg('count').reset_index()
        tmp.sort_values('article_id', ascending=False, inplace=True)
        tmp = tmp.groupby('customer_id').head(1).reset_index(drop=True)
        tmp.columns = ['customer_id', 'attribute_{}'.format(col), 'cnt']
        customers = customers.merge(tmp[['customer_id', 'attribute_{}'.format(col)]], on='customer_id', how='left')
    '''
...
def article_feature_engineer(samples, data, data_last_week, time_max):
    ...
    # tmp = data.groupby('article_id')['age'].agg(cols).reset_index()
    # tmp.columns = ['article_id'] + ['article_age_{}'.format(c) for c in cols]
...
import math
'''
def gen_sim_static_feats(data, samples):
    w2v_feats = [c for c in article_w2v_df.columns if c not in ['article_id']]
    l2norm1 = np.linalg.norm(article_w2v_df[w2v_feats], axis=1, keepdims=True)
    tmp = article_w2v_df.copy()
    tmp[w2v_feats] /= (l2norm1 + 1e-9)

    custs = data.customer_id.unique()

    sim_feats = []
    data = data.merge(tmp, on='article_id', how='left')
    for f in w2v_feats:
        data[f] = data[f].fillna(0)
    samp_df = samples[['customer_id', 'article_id']].merge(tmp, on='article_id', how='left')
    for cust in tqdm(custs):
        hist, cart = data[data.customer_id == cust], samp_df[samp_df.customer_id == cust]
        tmp_df = cart[w2v_feats].values.dot(hist[w2v_feats].values.T)
        tmp_feat = cart[['customer_id', 'article_id']]
        tmp_feat['w2v_sim_score_mean'] = np.mean(tmp_df, axis=1)
        tmp_feat['w2v_sim_score_max'] = np.max(tmp_df, axis=1)
        tmp_feat['w2v_sim_score_sum'] = np.sum(tmp_df, axis=1)
        tmp_feat['w2v_sim_score_std'] = np.std(tmp_df, axis=1)
        sim_feats.append(tmp_feat)
    sim_feats = pd.concat(sim_feats)

    print('生成w2v相似分数历史序列的统计特征:')
    print(sim_feats.shape)
    print(sim_feats.head())
    print('\n')

    samples = samples.merge(sim_feats, on=['customer_id', 'article_id'], how='left')

    del sim_feats, samp_df
    data.drop(columns=w2v_feats, inplace=True)
    gc.collect()
...
def interact_feature_engineer(samples, data, date_ths, data_lm, data_lw):
    ...
    for col in tqdm(cols):
        ...
        if col in att_cols:
            ...
            tmp = tmp.groupby('customer_id').head(1).reset_index(drop=True)
            tmp.columns = ['customer_id', 'attribute_{}'.format(col), 'cnt']
            cust_df = cust_df.merge(tmp[['customer_id', 'attribute_{}'.format(col)]], on='customer_id', how='left')
        '''
        '''
        if col in att_cols:
            tmp = data_[["customer_id", col, "t_dat"]].groupby(
                ["customer_id", col]).count().unstack().reset_index().fillna(0)
            tmp.columns = ['customer_id'] + ['{}-{}'.format(col, c[1]) for c in tmp.columns[1:]]
            tmp_cols = [c for c in tmp.columns if c not in ['customer_id']]
            tmp['sum_'] = tmp[tmp_cols].sum(axis=1)
            for c in tmp_cols:
                tmp[c] /= tmp['sum_']
            tmp.drop(columns=['sum_'], inplace=True)
            samples = samples.merge(tmp, on='customer_id', how='left')
        '''
    ...
    # 上周同属性人群购买候选物品次数
    # cols = ['age_group', 'attribute', 'postal_code']
    ...
    '''
    cols = ['attribute_{}'.format(c) for c in att_cols]
    samples = samples.merge(cust_df, on='customer_id', how='left')
    data_lw = data_lw.merge(cust_df[['customer_id'] + cols], on='customer_id', how='left')
    for col in tqdm(cols):
        tmp = data_lw.groupby([col, 'article_id'])['t_dat'].agg('count').reset_index()
        tmp.columns = [col, 'article_id', 'same_{}_purchase_corr_article_cnt_lw'.format(col)]
        new_cols += ['same_{}_purchase_corr_article_cnt_lw'.format(col)]
    ...
    for col in tqdm(cols):
        ...
        # samples['popularity_last_week_article_div_{}'.format(col)] = samples['popularity'] / samples[
        #     '{}_popularity_last_week'.format(col)]
...
def get_recall_samples(sim_dict, data, time_max, target_df, t1=10, t2=100):
...
def get_sim_item(df, user_col, item_col, recent_articles):
    # types = ['product_code', 'product_type_no', 'product_group_name', 'graphical_appearance_no',
    #          'colour_group_code', 'department_no', 'index_code', 'index_group_no', 'section_no', 'garment_group_no']
...
def construct_samples(data, date, last_days=7, recall_num=100, dtype='train'):
    ...
    print('待预测周正样本对数量:', len(
        target_df[target_df.customer_id.isin(target.customer_id.unique())].groupby(['customer_id', 'article_id'])[
            't_dat'].agg('count')))
    ...
    '''
    # BinaryNet进行召回
    binary_samples1= pd.read_csv('binary_recall/' + '{}_cg1.csv'.format(dtype), dtype={'article_id': str})
    binary_samples2 = pd.read_csv('binary_recall/' + '{}_cg2.csv'.format(dtype), dtype={'article_id': str})
    binary_samples = pd.concat([binary_samples1, binary_samples2])
    binary_samples.drop_duplicates(subset=['customer_id', 'article_id', 'label'], keep='first', inplace=True)
    del binary_samples1, binary_samples2
    gc.collect()
    samples = samples.merge(binary_samples[['customer_id', 'article_id', 'label']], on=['customer_id', 'article_id', 'label'], how='outer')
    # samples = samples.merge(binary_samples, on=['customer_id', 'article_id', 'label'], how='outer')
    print(samples.shape)
    print('BinaryNet召回后: ', samples.label.mean())

    # ItemCF进行召回
    itemcf_samples1 = pd.read_csv('itemcf_recall/' + '{}_cg1.csv'.format(dtype), dtype={'article_id': str})
    itemcf_samples2 = pd.read_csv('itemcf_recall/' + '{}_cg2.csv'.format(dtype), dtype={'article_id': str})
    itemcf_samples2 = itemcf_samples2.groupby('customer_id').head(100).reset_index(drop=True)
    itemcf_samples = pd.concat([itemcf_samples1, itemcf_samples2])
    itemcf_samples.drop_duplicates(subset=['customer_id', 'article_id', 'label'], keep='first', inplace=True)
    del itemcf_samples1, itemcf_samples2
    gc.collect()
    samples = samples.merge(itemcf_samples[['customer_id', 'article_id', 'label']], on=['customer_id', 'article_id', 'label'], how='outer')
    ...
    # cf召回
    t1, t2 = 20, 200  # 20, 300 # 20, 200
    pop_num = 6000
    last_date = 30
    last_date = datetime.datetime.strptime(begin_date, '%Y-%m-%d') - datetime.timedelta(days=last_date)
    recent_active_articles = set(list(dummy_dict.index[:pop_num]))
    sim_dict = get_sim_item(data_hist[data_hist.t_dat >= last_date], 'customer_id', 'article_id',
                            recent_active_articles)
    cf_samples = get_recall_samples(sim_dict, data_hist_, datetime.datetime.strptime(begin_date, '%Y-%m-%d'),
    ...
    # sim_dict = np.load('sim_dict/art_detail_sim_dict_offline.npy', allow_pickle=True).item()
    ...
    # samples = gen_cf_feats(samples, data_hist_, dtype)
    del data_hist_
    gc.collect()

    # samples.drop(columns=['source_article', 'cf_sim_score'], inplace=True)
    # samples.drop(columns=['cf_sim_score'], inplace=True)
    # samples['source_article'] = samples['source_article'].map(art_dic)
    ...
    # 过去一天popularity
    df = data_lw[data_lw.t_dat == begin_date]
    tmp = df.groupby('article_id')['customer_id'].agg('count').reset_index()
    tmp.columns = ['article_id', 'popularity_last_day']
    samples = samples.merge(tmp, on='article_id', how='left')

    tmp = df.groupby('customer_id')['article_id'].agg('count').reset_index()
    tmp.columns = ['customer_id', 'customer_purchase_cnt_last1day']
    samples = samples.merge(tmp, on='customer_id', how='left')

    # 过去三天popularity
    last_three_days = 3
    last_three_date = datetime.datetime.strptime(begin_date, '%Y-%m-%d') - datetime.timedelta(days=last_three_days)
    df = data_lw[data_lw.t_dat >= last_three_date]
    tmp = df.groupby('article_id')['customer_id'].agg('count').reset_index()
    tmp.columns = ['article_id', 'popularity_last_3days']
    samples = samples.merge(tmp, on='article_id', how='left')

    tmp = df.groupby('customer_id')['article_id'].agg('count').reset_index()
    tmp.columns = ['customer_id', 'customer_purchase_cnt_last3days']
    samples = samples.merge(tmp, on='customer_id', how='left')
    tmp = data_lw.groupby('customer_id')['article_id'].agg('count').reset_index()
    tmp.columns = ['customer_id', 'customer_purchase_cnt_last7days']
    samples = samples.merge(tmp, on='customer_id', how='left')
    tmp = data_lm.groupby('customer_id')['article_id'].agg('count').reset_index()
    tmp.columns = ['customer_id', 'customer_purchase_cnt_last30days']
    samples = samples.merge(tmp, on='customer_id', how='left')
    ...
    del data_hist, data_lw, target, data_lm, df
...
def ranker(train, valid):
    ...
    # train['weight'] = train.groupby('customer_id')['label'].transform('sum') + 1
    ...
    del_cols = ['weight']  # + list(tfidf_df.columns)[:5]# ['popularity', 'popularity_last_1days', 'popularity_last_month']
...
def get_customer_group(valid, begin_day):
    ...
    for cust_id, labels, hist_arts, hist_dates in tqdm(
            valid[['customer_id', 'label', 'article_id', 't_dat']].values):
...
def prediction(valid, dtype):
    ...
    print('[{}] MAP@12: '.format(dtype), map12)
...
# w2v_feats = [c for c in article_w2v_df.columns if c not in ['article_id']]
# l2norm1 = np.linalg.norm(article_w2v_df[w2v_feats],axis=1,keepdims=True)
# article_w2v_df[w2v_feats] /= (l2norm1+1e-9)

recall_num = 20 # 50  # 30 # 100
...
valid_ratio = 0.6
valid_custs = valid.customer_id.unique()
length = int(len(valid_custs) * valid_ratio)
import random
random.shuffle(valid_custs)
model, feats = ranker(train, valid[valid.customer_id.isin(valid_custs[:length])])

# tmp = valid.groupby('customer_id')['label'].agg('sum').reset_index()
# eff_custs = tmp.loc[tmp.label > 0]['customer_id'].unique()
# print('验证集有效(召回了目标项目)用户数量: ', len(eff_custs))
# model, feats = ranker(train, valid[valid.customer_id.isin(eff_custs)])
del train
gc.collect()

def Inference(model, data, feats, split_num=3):
    ...
    for i in tqdm(range(split_num + 1)):
        ...
        valid['prob'] = model.predict(valid[feats], num_iteration=model.best_iteration_)
...
valid = Inference(model, valid, feats, split_num=3)
...
res = valid_df.merge(tmp, on='customer_id', how='left')
prediction(res, 'all')
...
valid_cg3 = valid_df[valid_df.customer_id.isin(cg3)]
res = valid_cg3.merge(tmp, on='customer_id', how='left')
prediction(res, 'cg3')


save_path = 'offline_prob/1model/'
