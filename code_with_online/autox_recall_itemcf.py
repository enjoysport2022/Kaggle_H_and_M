...
def get_itemcf_recall(data, target_df, df,
    ...
    sim_item_corr, user_item_dict, user_time_dict, user_price_dict = get_sim_item(df, uid, iid, use_iif=use_iif, time_max=time_max)
    
    samples = []
    target_df = target_df[target_df.customer_id.isin(data.customer_id.unique())]

    for cust, hist_arts, dates in tqdm(data[[uid, iid, 't_dat']].values):
...
def itemcf_recall(uids, data, date, uid, iid, time_col, last_days=7, recall_num=100, dtype='train', topk=1000, use_iif=False, sim_last_days=14):
    ...
    if dtype == 'train':
        ...
        # target = target[target.t_dat >= sim_last_date]
        #
        # del target['t_dat']
        ...
        tmp = data_hist_.groupby(uid)['t_dat'].agg(list).reset_index()
        ...
        tmp = data_.groupby(uid)['t_dat'].agg(list).reset_index()
        df_hist = df_hist.merge(tmp, on=uid, how='left')

        samples = []
        for cust, hist_arts, dates in tqdm(df_hist[[uid, iid, 't_dat']].values):
