...
def gen_detail_content_recall_test(sim_dict, data_hist, topn=20, topk=100, prefix='detail'):
    def REC(sim_dict, hists, topn=20, topk=100):
        rank = {}
        for art in hists:
            if art not in sim_dict:
    ...
    df = data_hist.groupby('customer_id')['article_id'].agg(list).reset_index()

    samples = []
    for cust, hists in tqdm(df.values):
        if hists is np.nan:
            continue
        rec = REC(sim_dict, hists, topn, topk)
        # rec = [k for k, v in rec]
    ...
    samples = pd.DataFrame(samples, columns=['customer_id', 'article_id', '{}_content_sim_score'.format(prefix)])
...
def construct_samples(data, date, last_days=7, recall_num=100, dtype='valid'):
    ...
    samples = samples.merge(dc_samples[['customer_id', 'article_id', 'label']],
...
def construct_samples_test(custs, data, date, last_days=7, recall_num=100):
    ...
    # w2v content recall
    print('w2v content召回')
    sim_dict = np.load('sim_dict/w2v/sim_dict_test.npy', allow_pickle=True).item()
    # dc_samples = gen_detail_content_recall(sim_dict, target, data_hist_, topn=20, topk=100, prefix='w2v')
    dc_samples = gen_detail_content_recall_test(sim_dict, data_, topn=20, topk=100, prefix='w2v')
    samples = samples.merge(dc_samples, on=['customer_id', 'article_id'], how='outer')
    print(samples.shape)
    del dc_samples
    gc.collect()

    # detail content recall
    print('detail content召回')
    # sim_dict = np.load('sim_dict/art_detail_sim_dict_v1.npy', allow_pickle=True).item()
    sim_dict = np.load('sim_dict/art_detail_sim_dict_test.npy', allow_pickle=True).item()
    dc_samples = gen_detail_content_recall_test(sim_dict, data_, topn=20, topk=100, prefix='detail')
    # samples = samples.merge(dc_samples[['customer_id', 'article_id']], on=['customer_id', 'article_id'], how='outer')
    samples = samples.merge(dc_samples, on=['customer_id', 'article_id'], how='outer')
    print(samples.shape)
    del dc_samples
    gc.collect()
    ...
    last_days = [1, 3, 7, 14]
    for last_day in tqdm(last_days):
        last_date = datetime.datetime.strptime(date, '%Y-%m-%d') - datetime.timedelta(days=last_day)
        tmp = data[data.t_dat > last_date].groupby('article_id')['customer_id'].agg('count').reset_index()
        tmp.columns = ['article_id', 'popularity_last_{}days'.format(last_day)]
        samples = samples.merge(tmp, on='article_id', how='left')
...
# Inference
def Inference(model, feats, transactions_train, batch_size=30000, split_num=3):
